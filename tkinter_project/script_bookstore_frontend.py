from tkinter import *
from script_bookstore_backend import Database
...
class Window(object):
    def __init__(self, window):
        ...
        b3 = Button(self.window, text="Add entry",
                    width=12, command=self.add_command)
        b3.grid(row=4, column=3)

        b4 = Button(self.window, text="Update", width=12,
                    command=self.update_command)
        b4.grid(row=5, column=3)

        b5 = Button(self.window, text="Delete", width=12,
                    command=self.delete_command)
        b5.grid(row=6, column=3)

        b5 = Button(self.window, text="Close",
                    width=12, command=window.destroy)
